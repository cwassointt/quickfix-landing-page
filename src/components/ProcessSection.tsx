...
// ── LED SPEC (replaces TypewriterSpec) ───────────────────────────
...
// ── GLOW SWEEP (one-time on mount, left→right) ────────────────────
...
const useCyclingSteps = (total: number, sectionInView: boolean) => {
  ...
  useEffect(() => {
    ...
    // First run: light up 1→4 sequentially
    let step = 0;
    const advance = () => {
      setActive(step);
      step++;
      if (step < total) {
        ...
        // After first pass, cycle continuously every 6s
        ...
        // keep cycling — store for cleanup
